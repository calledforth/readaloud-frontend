...
import { health } from '../lib/provider/mock';
...
function Flower({ state }: { state: Status }) {
  const color = state === 'connected' ? '#22C55E' /* parrot/emerald */ : state === 'checking' ? '#A3E635' : '#F87171';
  const animate = state === 'connected' ? 'animate-[spin_5s_linear_infinite]' : state === 'checking' ? 'animate-[spin_2.5s_linear_infinite]' : '';
  return (
